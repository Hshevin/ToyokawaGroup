...
fun ReviewScreen(
    ...
    val selected = uiState.anomalies.firstOrNull { it.id == uiState.selectedAnomalyId }
    ...
    Box(modifier = modifier.fillMaxSize()) {
        Column(
            ...
            Text(
                activeTask?.let { "当前任务：${it.name} · ${it.status.label}" }
                    ?: "请先在任务页创建任务",
            ...
            Box(
                ...
                Canvas(Modifier.fillMaxSize()) {
                    uiState.anomalies.forEach { anomaly ->
                        val color = if (anomaly.id == selected?.id) {
                            Color(0xFFFFC107)
                        } else {
                            Color(0xFF4CAF50)
                        }
            ...
            Text(
                text = if (uiState.anomalies.isEmpty()) {
                    "暂无候选，请先在影像页导入并完成检测。"
                } else {
                    "点击绿色框或下方列表打开建筑详情；拖拽画布可新增框选。"
                },
            ...
            if (uiState.anomalies.isNotEmpty()) {
                Text(
                    text = "全部候选（${uiState.anomalies.size}）",
                ...
                LazyColumn(
                    ...
                    items(uiState.anomalies, key = { it.id }) { anomaly ->
                        val isSelected = anomaly.id == selected?.id
                        Card(
                            ...
                            Row(
                                ...
                                Column(verticalArrangement = Arrangement.spacedBy(2.dp)) {
                                    ...
                                    Text(
                                        "${anomaly.reviewStatus.label} · ${anomaly.source}",
                                        style = MaterialTheme.typography.bodySmall,
                                    )
                                }
                            }
...
private fun AnnotationSheetContent(
    ...
    Column(
        ...
        Text("建筑详情", style = MaterialTheme.typography.titleLarge)
        Text(
        ...
        Row(
            ...
            Button(
                ...
                enabled = anomaly.reviewStatus != ReviewStatusUi.CONFIRMED,
            ) {
                Text("确认标注")
            }
            OutlinedButton(
                onClick = onReject,
                modifier = Modifier.weight(1f),
                enabled = anomaly.reviewStatus != ReviewStatusUi.REJECTED,
        ...
        OutlinedTextField(
            ...
            label = { Text("位置") },
            placeholder = { Text("可编辑；GeoTIFF 为经纬度，普通图为画面坐标") },
            supportingText = {
                Text("手动画框或重新检测后会自动填入，也可手动修改")
            },
            singleLine = true,
